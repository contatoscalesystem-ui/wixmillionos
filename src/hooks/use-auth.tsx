...
const Ctx = createContext<AuthCtx>({ session: null, loading: true, profile: null, role: null, refreshProfile: async () => {} });
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadProfile = async (uid?: string) => {
    if (!uid) { setProfile(null); setRole(null); return; }
  ...
  return (
    <Ctx.Provider value={{ session, loading, profile, role, refreshProfile: () => loadProfile(session?.user.id) }}>
      {children}
    </Ctx.Provider>
  );
}
