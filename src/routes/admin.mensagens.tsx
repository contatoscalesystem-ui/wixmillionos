...
const TYPES = [["information", "Informação"], ["warning", "Aviso"], ["important", "Importante"], ["maintenance", "Manutenção"]] as const;
...
function MessagesPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setTitle(""); setMessage(""); void qc.invalidateQueries({ queryKey: ["admin", "notifications"] });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={submit} className="grid max-w-2xl gap-4 rounded-lg border bg-card p-5">
        ...
        <Button type="submit" disabled={busy} className="w-fit bg-gold text-gold-foreground hover:bg-gold/90">{busy ? "Enviando..." : "Enviar mensagem"}</Button>
      </form>
