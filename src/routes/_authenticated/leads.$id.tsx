...
import { Archive, ArchiveRestore, ArrowLeft, Calendar, Copy, Globe, Instagram, MapPin, MessageCircle, Pencil, Send, Trash2, X, AlertTriangle } from "lucide-react";
...
function LeadPage() {
  ...
  const convert = async () => {
    ...
    // Single database transaction: client + lead status + activity, or nothing.
    const { data: clientId, error } = await supabase.rpc("convert_lead_to_client", { _lead_id: lead.id });
    if (error || !clientId) return toast.error(friendlyError(error));
  ...
  const createProject = async () => {
    ...
    if (error) { console.error(error); return toast.error("O cliente foi criado, mas não foi possível criar o projeto de site. Crie o projeto pela tela Produção."); }
  ...
  const toggleArchive = async () => {
    setArchOpen(false);
    const restoring = !!lead.archived_at;
    if (!(await updateLead(restoring ? { archived_at: null, archived_by: null } : { archived_at: new Date().toISOString(), archived_by: session?.user.id ?? null }))) return;
    await logActivity(lead.id, restoring ? "restored" : "archived", restoring ? "Lead restaurado" : "Lead arquivado");
      ...
      <div className="rounded-lg border bg-card p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            {lead.archived_at && <p className="mt-1 text-sm text-muted-foreground">Arquivado em {fmtDate(lead.archived_at, true)}</p>}
